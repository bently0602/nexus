...
function TableNode({ data }: NodeProps<Node<TableNodeData>>) {
  ...
  return <div className="schema-table-node" style={{ borderColor: table.color }}><header style={{ background: table.color }}>{table.name || "Untitled table"}</header>{table.description ? <p>{table.description}</p> : null}<div>{table.columns.map((column) => <div className="schema-column" key={column.id} title={column.description || undefined}><Handle type="target" position={Position.Left} id={`target-${column.id}`} /><span><b>{column.name || "column"}</b> <em>{column.type}</em></span><span>{[column.primaryKey ? "PK" : column.unique ? "UQ" : "", column.identity ? "ID" : ""].filter(Boolean).join(" ")}</span><Handle type="source" position={Position.Right} id={`source-${column.id}`} /></div>)}</div></div>;
}
...
export function SqlSchemaHostApp() {
  ...
  function addTable() {
    ...
    update((draft) => { draft.tables.push({ id, name: "new_table", description: "", color: COLORS[draft.tables.length % COLORS.length], position: { x: 80 + draft.tables.length * 40, y: 80 + draft.tables.length * 40 }, columns: [{ id: crypto.randomUUID(), name: "id", type: "uuid", description: "", primaryKey: true, nullable: false, unique: false, identity: "", defaultValue: "" }] }); });
    setSelectedTableId(id);
  }
  function addColumn() { if (!selected) return; update((draft) => draft.tables.find((table) => table.id === selected.id)?.columns.push({ id: crypto.randomUUID(), name: "column", type: "text", description: "", primaryKey: false, nullable: true, unique: false, identity: "", defaultValue: "" })); }
